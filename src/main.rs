...
use ensemble::fusion::{BayesianFusion, kalman_signal, frequency_signal, orderbook_signal};
use estimation::{Observation, StateEstimator, dual_kalman::DualKalmanFilter};
use execution::{executor::Executor, model::{ExecutionModel, ImpactParams}};
use features::frequency::FrequencyExtractor;
...
async fn main() -> Result<()> {
    ...
    // ── Per-asset online estimator state ──────────────────────────────────────
    let assets = universe();
    ...
    loop {
        ...
        // ── Per-asset: update online estimators → build AssetDistributions ────
        ...
        for asset in &assets {
            ...
            let (price, volume) = {
                let s = store.read();
                let bar = s.bars_for(sym, "5m").and_then(|b| b.back());
                (bar.map(|b| b.close).unwrap_or(0.0), bar.map(|b| b.volume).unwrap_or(0.0))
            };
            if price <= 0.0 { continue; }

            // EWMA vol (for kill-switch monitoring).
            let log_ret = store.read().returns(sym, "5m").last().copied().unwrap_or(0.0);
            if let Some(ev) = ewma_vols.get_mut(sym) { ev.update(log_ret); }

            // ── Dual Kalman Filter (state + parameter tracking) ───────────────
            let obs = Observation { price, volume, timestamp_ms: timestamp_now_ms() };
            ...
            // ── Frequency extractor (Goertzel DFT, online) ───────────────────
            let freq_feat = freq_extractors.get_mut(sym).unwrap().update(price);

            // ── Hawkes process (order-book dynamics) ─────────────────────────
            let hawkes_opt = estimate_hawkes(&store, sym, &mut hawkes_models);

            // ── Bayesian fusion of the three probabilistic signal sources ─────
            let ks = kalman_signal(&posterior);
            let fs = freq_feat.as_ref().map(|f| frequency_signal(f, posterior.velocity));
            let os = hawkes_opt.as_ref().map(|h| orderbook_signal(h));

            let fusion = bayesian_fusions.get_mut(sym).unwrap();
            if let Some(dist) = fusion.fuse(&[Some(ks), fs, os]) {
                ...
                asset_dists.push(AssetDistribution {
                    ...
                    estimated_tc_bps: spread_bps + 2.0,
                });
            }
        }

        // ── Joint combo optimization (Kelly + cardinality + TC budget) ────────
        ...
        info!(
            "Combo: {} leg(s), ELG={:.5}, TC={:.2}bps, regime={}",
            combo.n_legs(), combo.total_expected_log_growth,
            combo.total_tc_bps, combo.regime_summary()
        );
        ...
        for leg in &combo.legs {
            ...
            match risk_mgr.check_signal(&signal, &portfolio, None) {
                Ok(()) => {
                    ...
                    if exec_metrics.is_executable {
                        ...
                        warn!("Execution rejected for {}: {}", leg.asset, exec_metrics.rejection_reason);
                    }
                }
                Err(e) => warn!("Risk check failed for {}: {}", leg.asset, e),
            }
        }
...
/// Convert an `OrderLeg` to a `TradeSignal` for `RiskManager` and `Executor`.
fn leg_to_signal(leg: &OrderLeg, asset_index: usize) -> TradeSignal {
    ...
    TradeSignal {
        ...
        generated_at_ms: timestamp_now_ms(),
...
fn estimate_hawkes(
    ...
    let (buy_times, sell_times) = {
        ...
        (hist.buy_times_ms.iter().copied().collect::<Vec<_>>(),
         hist.sell_times_ms.iter().copied().collect::<Vec<_>>())
    };
    if buy_times.len() < 10 || sell_times.len() < 10 {
        return hawkes_models.get(asset).cloned();
    }
    match HawkesProcess::fit(&buy_times, &sell_times) {
        Ok(h) => { let c = h.clone(); hawkes_models.insert(asset.to_string(), h); Some(c) }
...
fn log_portfolio_state(portfolio: &PortfolioState) {
    info!(
        ...
        portfolio.nav, portfolio.leverage_ratio(),
        portfolio.drawdown() * 100.0, portfolio.n_open_positions(),
    );
}
